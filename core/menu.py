...
from core.lib.prompts import main_color, second_color, support_color
from core.lib.prompts import PromptsMenu
...
class Menu:
    ...
    def __container(self) -> None:
        ...
        while self.__stack:
            ...
            self.__console.print(PromptsMenu.choise)

            choice = self.__console.input(Prompts.arrows)  # Сразу, чтобы не писать input в скрипте.
    ...
    def __choice_handler(self, options: list[str], choice: str) -> bool:
        ...
        if index is None:
            self.__console.print(PromptsMenu.bad_entry)
        ...
        if index == len(options) + 1:  # Если назад:
            if len(self.__stack) > 1:  # Если длина списка больше чем 1, (но не равна 1)
                ...
                self.__console.print(PromptsMenu.main_menu)
            return False
        elif index == len(options) + 2:  # Выход - возвращем True, цикл рвётся
            self.__console.print(PromptsMenu.exit)
            return True
        else:  # В иных случаях (при выборе другого меню или действия/заглушки)
            selected = options[
                index - 1
            ]  # Координируем индексы, чтобы был выбран верный вариант, а не на +1
            ...
            # Если внутри ключ - строка, а !!значение!! - !!ссылка на функцию!!
                ...
                if action:  # Выполняем действие
                    ...
                    self.__console.print(f"{PromptsMenu.perform_action} {selected}")
    ...
    def __display_menu(self, title: str, structure: list[str]) -> None:
        ...
        self.__support_console.rule(f"[{main_color}]{title}[/{main_color}]", style=second_color)  # Отрисуем титульник
        for number, item in enumerate(structure, 1):  # Нумеруем
            self.__console.print(Text(f"{number}. ", style=support_color) + Text(item, style=main_color))
        # И отрисуем

        # "Назад" и "Выход" добаляем как в самом конце всегда
        self.__console.print(Text(f"{len(structure)+1}. ", style=support_color), end="")
        self.__console.print(PromptsMenu.back)
        self.__console.print(Text(f"{len(structure)+2}. ", style=support_color), end="")
        self.__console.print(PromptsMenu.exit)
