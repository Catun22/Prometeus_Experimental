...
def ansi_code_fixer() -> None:
    ...
    ENABLE_VIRTUAL_TERMINAL_PROCESSING = (
        0x0004  # Флаг для включения ANSI escape-последовательностей
    )
    ...
    if sys.platform == "win32":
        ...
        if ver.major > 10 or (ver.major == 10 and ver.build >= 10586):
            ...
            print(
                "Ваша версия Windows слишком старая, ANSI escape-последовательности не поддерживаются."
            )
...
def install() -> None:
    ...
    try:  # Пытаемся вызвать функцию, ремонтирующую терминал Win, для отображения цветов
        ...
        ImportError
    ):  # Ловим ошибки при импорте - если ошибка есть, значит colorama нет
        print(prompt)
        if sys.platform == "win32":
            ansi_code_fixer()
