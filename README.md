# alpha v.0.1.1 экспериментального проекта Prometeus

Вход в программу находится в `__main__.py`. 

> По желанию вы можете установить colorama с помощью: **`pip install -r requirements.txt`**.

### Изменения в проекте:

В `__main__.py` убран второй вызов функции `clean()`.
...
Благодаря этому небольшому изменению в терминале появится следующее:
```
Библиотека colorama не установлена.
Используем встроенный механизм системы ctypes для поддержания ANSI escape code.

Поддержка ANSI escape включена.
Красный текст и зелёный текст
```

> Примечание: "Красный текст и зелёный текст" действительно будут раскрашены в красный и зеленый соовтетственно.

Также был изменен `installer.py`, который теперь использует модуль `ctypes`, если не установлена `colorama`. Из ` installer.py` удалена автоматическая установка `colorama`.
